...
import { getOpsOverview } from "@/lib/services/partners";
import {
...
export default async function OpsPage() {
  ...
  const ops = await getOpsOverview(context.propertyId);

  return (
    <div className="mx-auto flex w-full max-w-[860px] flex-col gap-6">
      <section className="rounded-[24px] border border-[#e8e0d4] bg-white px-6 py-6 shadow-sm">
        <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[#8c7e6a]">Ops</p>
        <h1 className="mt-2 font-serif text-[30px] font-semibold tracking-tight text-[#2c2416]">Background work</h1>
        <p className="mt-3 text-[14px] text-[#8c7e6a]">
          Pipeline health, outreach infrastructure, sources, and research triage.
        </p>
      </section>
